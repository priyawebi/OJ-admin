...
useEffect(() => {
  if(router.query.id){
    axios.get(`${url}api/admin/jewellerytype/${router.query.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {setData(res?.data?.name),setLogo(`${url}`+res.data.image)});
  }
}, [router.query.id]);
  ...
  return (
    <>
      ...
      <div className="mt-4 m-4">
        ...
        <Typography color="textPrimary" >
        <Link color="inherit"  href="/all-jewelery-type" >
          <h3 className='dynamic-breadcumbs'>Jewellery Type</h3>
          </Link>
          </Typography>
      ...
      <Formik
          ...
          onSubmit={(values) => {
            ...
            if (router.query.id === undefined) {
              axios
                .post(`${url}api/admin/jewellerytype`,formData,{
                  headers: {
                ...
                .then((result) => {
                  showToaster("success","JewelType Added Successfully");
                })
                .catch((err) => {
                  showToaster("error","JewelType Not Inserted");
                });
        
                Router.push("/all-jewelery-type");
            } else {
             console.log(formData)
              axios
                .put(`${url}api/admin/jewellerytype/${router.query.id}`,formData,{
                  headers: {
                    "Content-Type": "multipart/form-data",
                    Authorization: `Bearer ${token}`,
                  },
                ...
                .then((result) => {
                  showToaster("success","JewelType Update Successfully");
                })
                .catch((err) => {
                  showToaster("error","JewelType Not Update ");
                });
            }            
            Router.push("/all-jewelery-type");
          }}
            ...
            <div className="cards">
              <div className="row align-items-center ">
                <div className="col-sm-3">
                  <h6 className="label-text bold">Jewel Type</h6>
                </div>
              </div>
