...
function BookingsList() {
    ...
    const clickAction = (data: any) => {
        Router.push("/booking-details?id=" + data.id);
    }
    ...
    const getCustomerData = (limit: number, page: number) => {
        fetch(`http://localhost:3000/bookings?_limit=${limit}&_page=${page}`)
            .then((res) =>
                res.json())
            .then((response) => {
                if (response && response.length) {
                    response.forEach((e: { status: String; }, i: number) => {
                        response[i].statusHtml = setStatusView(e.status);
                        response[i].clickAction = clickAction;
                    })
                    setTableData(response);
                    const totalRecord = 64;
    ...
    return (
        <>
            ...
            <div className='main-content'>
                <div className='m-4'>
                    <PageHead title="Bookings" smallCountValue={10} setOpen={setOpen} open={open} FilterButton={true} exportButton={true}/>
                    <div>
                        <Collapse in={open}>
                            <div id='example-collapse-text'>
                                ...
                                <div className='col-sm-12 row align-items-center mt-3'>
                                    ...
                                    <div className='col-sm-4 d-flex gap-2 align-items-center mt-4'> Location
